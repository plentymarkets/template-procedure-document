...
Mit dem Plugin-Modul ist es möglich, das plentysystems System um zusätzliche Funktionen zu erweitern und individuell zu gestalten. Bei Plugins handelt es sich um Erweiterungen des plentysystems Systems. So ist es etwa möglich, neue Zahlungsarten zu einem Webshop oder eine neue Schnittstelle zum Backend hinzuzufügen.
...
Wenn ein Plugin erworben wird, wird von plentysystems automatisch ein Auftrag an der entsprechenden Kunden-ID hinterlegt (siehe Kapitel <<#_kontakt_ids>>). Sind einer Kunden-ID mehrere plentysystems Systeme zugeordnet, steht das Plugin in all diesen Systemen zu Verfügung.

Eine Liste aller auf plentyMarketplace angebotenen Plugins ist im Anhang plentyMarketplace-Plugins aufgeführt.

== Git

Plugins können dem System über die Git-Dienste GitHub, Bitbucket und GitLab hinzugefügt werden. Das Repository, das hinzugefügt werden soll, muss eine https://developers.plentymarkets.com/dev-doc/plugin-information[valide Plugin-JSON^] enthalten.
...
Beim Installieren von Plugins, die auf plentyMarketplace erworben wurden, wird der beim Kauf angelegte Auftrag von plentysystems überprüft. Anschließend werden die Dateien des Plugins in das AWS-Verzeichnis des Kunden kopiert.

Nach dem Installieren können Anwender Plugins innerhalb eines Plugin-Sets aktivieren und deaktivieren. Nur aktivierte Plugins werden beim Bereitstellen berücksichtigt. Nachdem ein Plugin bereitgestellt wurde, kann es aktiviert und deaktiviert werden. Wenn ein Plugin bereitgestellt und deaktiviert ist, wird der Code des Plugins nicht ausgeführt. Damit der Code ausgeführt wird, muss das Plugin aber nicht erneut bereitgestellt, sondern nur aktiviert werden.
...
Alle notwendigen Schritte zur Einrichtung werden vom Anwender anhand einer Anleitung vorgenommen, die vom Anbieter des Plugins zu Verfügung gestellt wird.
