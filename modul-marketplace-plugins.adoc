...
Wenn ein Plugin erworben wird, wird von plentymarkets automatisch ein Auftrag an der entsprechenden Kunden-ID hinterlegt (siehe Kapitel <<modul-crm, Kontakt-IDs>>). Sind einer Kunden-ID mehrere plentymarkets Systeme zugeordnet, steht das Plugin in all diesen Systemen zu Verfügung.
