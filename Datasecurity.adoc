...
===== Systemdaten
Die Speicherung systembezogener Daten eines plentymarkets Kundensystems erfolgt in einer eigenen Datenbank. Hier werden z.B. Daten wie
Kundenname, Anschrift etc.
* Artikel (inkl. aller notwendigen Daten)
* Kategorien
* Angebundene Marktplätze etc.
gespeichert.
...
* Domains
* IP-Adressen
* Speicherort für DB und NFS
etc. werden in einer zentralen Datenbank gespeichert, auf die nur das Mastersystem von plentymarkets zugreifen kann. Diese Daten werden zur Verwaltung der Kundensystem genutzt.
...
Welche Daten werden wie gespeichert?
Die Speicherung der Daten eines plentymarkets Kundensystems erfolgt u.a. auf Amazon S3-Servern. Hier werden z.B. Daten wie
...
Backup-Server
Alle oben genannten Kernsysteme sind wie erwähnt redundant ausgelegt. Zusätzlich werden alle Datenbank- und NFS-Daten auf 4 Backup-Servern gesichert. Die Datenbank-Daten werden täglich, die NFS-Daten werden einmal wöchentlich (Sonntag Nachts) gesichert.
Recovery
Die Kunden habe die Möglichkeit Backups selbstständig über den Admin-Bereich (Grundeinstellungen => Backups)  wieder einzuspielen. Bei Bedarf spielt die Administration Backups oder Teile von Backups wieder ein.
Löschung
Systemdaten
...
Im Anschluss werden alle Systemdaten auf den Backup-Servern gesichert, und im Anschluss von den Produktiv-Servern gelöscht. Die gesicherten Daten werden weitere 30 Tage aufbewahrt und anschließend endgültig gelöscht. Dabei werden die Daten mit d
Kundendaten
...
==== Neusysteme
Für Neukunden stehen die Standorte “Amazon Frankfurt” sowie “Amazon Dublin” zu Auswahl.
