...
<<tabelle-liste-integrierte-versanddienstleister>> listet die Versanddienstleister, zu denen eine Schnittstelle besteht oder denen man über das System eine Polling-Datei zur Verfügung stellt, auf.
...
Ein Beispiel für eine Zollinhaltserklärung und eine Paketkarte für DHL sind in <<#_anhang_dhl_paketkarte_und_zollinhaltserklärung, Anhang DHL Paketkarte und Zollinhaltserklärung>> zu finden.

== Anhänge

=== Anhang Lieferländer mit ID

<<tabelle-lieferlaender-mit-id>> listet die im System voreingestellten und für den Anwender nicht änderbaren Ländernamen sowie deren IDs auf.
