...
=== Allgemeines
plentysystems ist durch den TÜV "Geprüfte Cloud-Sicherheit" zertifiziert. Die Sicherheitsarchitektur entspricht dem aktuellen Stand der Technik.
...
===== Kundendaten
Die eigentlichen Kundendaten und einige Kundensystemdaten werden in einer zentralen Datenbank gespeichert, auf die nur das Mastersystem von plentysystems zugreifen kann. Diese Daten werden zur Verwaltung der Kundensystem genutzt.
...
==== Allgemeines
Andere Daten eines plentysystems Kundensystems werden in Filestrukturen abgelegt. Hierzu wird das Netzwerkprotokoll NFS genutzt. Alle NFS-Server sind redundant aufgestellt. Ein Zugriff auf die NFS-Server ist lediglich innerhalb des Clusters möglich. Ein Zugriff von Außerhalb ist nicht möglich.

==== Welche Daten werden wie gespeichert?

===== Systemdaten
Die Speicherung der Daten eines plentysystems Kundensystems erfolgt u.a. auf NFS-Servern. Hier werden z.B. Daten wie
...
==== Allgemeines
Andere Daten eines plentysystems Kundensystems werden in Filestrukturen abgelegt. Hierzu wird der Amazon S3 Dienst genutzt. Alle S3-Server sind redundant aufgestellt. Ein Zugriff auf die S3-Server ist lediglich innerhalb der Cloud möglich. Ein Zugriff von außerhalb ist nicht möglich.

==== Welche Daten werden wie gespeichert?
Die Speicherung der Daten eines plentysystems Kundensystems erfolgt u.a. auf Amazon S3-Servern. Hier werden Daten gespeichert wie:
...
=== Systemdaten
Systemdaten (Datenbank, NFS-Daten und S3-Daten) werden 30 Tage nach Ablauf des Vertrages des plentysystems Systems zur Löschung freigegeben. Hierbei hat der Kunde 30 Tage nach Ablauf Zeit seinen Vertrag zu reaktivieren, ohne das dass System aufwändig wiederhergestellt werden muss.
